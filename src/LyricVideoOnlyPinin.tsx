...
const LyricLine: React.FC<{ line: LineData; onlyPinyin?: boolean }> =
  React.memo(({ line, onlyPinyin = false }) => (
    <>
      <AbsoluteFill
        style={
          onlyPinyin ? styles.screenContainerOnlyPinyin : styles.screenContainer
        }
      >
        <div style={styles.sentenceRow}>
          {line.words.map(
            (word, wIndex) =>
              (!onlyPinyin || word.hanzi) && (
      ...
      {lyricsData.lines.map((line) => {
        ...
        return (
          <Sequence key={line.id} from={startFrame} durationInFrames={duration}>
            ...
            <LyricLine line={line} onlyPinyin />
          </Sequence>
        );
      })}
